...
const GooglePlay = function GooglePlay(opts) {
    ...
    function getDownloadInfo(pkg, versionCode) {
        return getDownloadPurchaseInfo(pkg, versionCode)
            .then((purchaseInfo) => {
                if (purchaseInfo.downloadSize === null) {
                    return getDownloadDeliveryInfo(pkg, versionCode);
                } else {
                    return purchaseInfo;
                }
            })
     ...
     * @param {Number} versionCode
     */
    function getDownloadPurchaseInfo(pkg, versionCode) {
     ...
     * @param {Number} versionCode
     */
    function getDownloadDeliveryInfo(pkg, versionCode) {
        return executeRequestApi('delivery', {ot: 1, doc: pkg, vc: versionCode}).then(function (res) {
