...
var GooglePlay = (function GooglePlay(username, password, androidId, useCache, debug) {
  ...
  var schema = new Schema(fs.readFileSync(__dirname+'/gen/googleplay.desc'));
 ...
 var memoizedExecuteRequestApi2 = USE_CACHE ?
  ...
  function getBulkDetails(packages) {
    var data = schema.BulkDetailsRequest.serialize({
    ...
    .then(function (res) {
      return res.payload.bulkDetailsResponse.entry;
    });
  }
  ...
  function getDeliveryData(pkg, vc) {
    return executeRequestApi('delivery', {doc: pkg, vc: vc, ot: 1}).then(function (res) {
